...
dataset['target_week1'] = (dataset.groupby(['Commodity', 'Market'])['Modal_Price'].shift(-2))
...
model_week2 = RandomForestRegressor(
    n_estimators=10,
    random_state=0
)
model_week2.fit(X_train, y_train)
y_pred = model_week2.predict(X_test)
import pickle

pickle.dump(model_week2, open('model_week2.pkl', 'wb'))

print("Week 2 model saved!")