...
dataset['target_week1'] = (dataset.groupby(['Commodity', 'Market'])['Modal_Price'].shift(-4))
...
model_week4 = RandomForestRegressor(
    n_estimators=10,
    random_state=0
)
model_week4.fit(X_train, y_train)
y_pred = model_week4.predict(X_test)
import pickle

pickle.dump(model_week4, open('model_week4.pkl', 'wb'))

print("Week 4 model saved!")